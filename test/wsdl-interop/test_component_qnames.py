#!/usr/bin/env python3
"""Independent XSD 1.0 schema component QName resolution.
...
class ComponentQNamesTest(unittest.TestCase):
    def test_pinned_xerces(self):
        models = json.loads((Path(__file__).parent / 'fixtures/component-qnames.json').read_text())
        self.assertEqual(91, len(models))
        self.assertEqual(52, sum(m['schema_valid'] for m in models))
        self.assertEqual(len(models), len({m['name'] for m in models}))
        report = run([SchemaJob(m['name'], f'http://example.invalid/component-qnames/{i}.xsd',
