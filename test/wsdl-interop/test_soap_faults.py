#!/usr/bin/env python3
"""SOAP 1.2 fault grammar. Copyright (C) 2026 Qore Technologies, s.r.o."""
import hashlib
import json
...
class SoapFaultTests(unittest.TestCase):
    ...
    def test_client_fault_validation_and_recovery(self):
        ...
        cases = self.cases + [self.cases[0]]
        for _ in range(6):
            for row in cases:
                replies.put(row)
        def reply(request):
            root = etree.fromstring(request['body'])
            self.assertEqual(f"{{{URI['12']}}}Envelope", root.tag)
            self.assertEqual('invoice-71',root.find(f"{{{URI['12']}}}Body/{{urn:soap-envelope-test}}value").text)
            return 500, {}, replies.get_nowait()['xml'].encode()
        with origin('faults', requests, reply, content_type='application/soap+xml') as url:
            result = subprocess.run(['qore','-b','--enable-debug',str(ROOT / 'client.qr')],
                input=json.dumps({'url':url,'count':len(cases)}),capture_output=True,text=True,env=ENV,timeout=90)
