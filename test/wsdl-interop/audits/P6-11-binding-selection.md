# P6-11 binding selection audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.codex/skills/audit-changes/SKILL.md` to the selected-binding
lookup correction, test and documentation. Qore module structure, sandboxing and
cancellation guides were checked for applicability. No C++ or provider metadata
changes are present.
...
| 9 | Copyright 2026 on all new files | Pass | New test and evidence files carry 2026 copyright; WSDL retains its 2026 notice. |
...
| 13 | `%modern` directive present | Pass | Both changed tests declare %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/wsdl-binding-selection.qtest and test/soap.qtest are executable (0755). |
...
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | No new external dependency; core QUnit uses hard requires and XML belongs to this repository. |
...
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | No production I/O added. SoapClient fixtures generate messages locally; invalid calls reject before I/O. Existing broad loopback integration tests cover successful calls. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | The fix uses the existing operation binding association, without a parallel registry or fallback. No stub or test relaxation. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Lookup validates the existing association before returning. It mutates no object or registry; repeated valid lookups still succeed after errors. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Read-only lookup adds no state. Existing construction-before-sharing rules remain unchanged. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | The returned declaration is explicitly typed WSOperation and the binding uses its canonical component key. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | One existing hash lookup via WSOperation::getBinding(); no scan or copied membership structure. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Tests distinguish missing binding, absent port-type operation, omitted concrete operation, empty binding, unknown namespace and ambiguous imported local binding names. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Public parameters, return value, error categories, note and example document selected binding membership. Release notes and durable component design are updated; WSDL docs build cleanly. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No I/O, new buffers, credentials, formatting or mutable global state. Selected binding membership is enforced before returning a handle. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | Baseline fails both direct and imported membership checks. The existing postForm test is corrected to its declared POST binding b3 and gains a negative GET-binding assertion. Final focused tests pass 3 cases / 134 assertions; broad Qore integration and astparser results are recorded in P6-11-validation.json. |

All 62 checks: **18 Pass / 44 N/A / zero Fail**.

See [validation](../P6-11-validation.json) and [evidence](../binding-selection-evidence.md).
