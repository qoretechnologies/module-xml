# Gate hang-guard recalibration audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to the uncommitted changes on main `develop`
based on 8f1f8b2. Scope: hang-guard limits in `test_list_values.py`, `test_corpus.py`,
`test_numeric_constraints.py` and `test_wsdl_calendar_constraints.py`; the survey worker default in
`survey.py`, `coverage.py` and `archive_roles.py`; the matching `test_survey.py` assertion; `README.md`,
`PLAN.md` (P9 benchmark item) and `EXECUTION.md`. No production code changed.
...
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Every guard below 2.5x headroom over the maximum measured across all 178 gates is raised to at least 3.5x. P9 now tracks performance with a benchmark item instead of relying on timeouts. |
...
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | The README documents the new 150 s default in both places; the list-values guard carries a comment with its measured cost; `EXECUTION.md` has the measurement table; PLAN.md P9 has the benchmark item. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | All nine affected gates pass: list values, regex classes, IEEE facets, corpus, survey, coverage, archive roles, and numeric and calendar constraints. |
