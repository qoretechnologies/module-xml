# P9g-01 QName AOT stack root cause record audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` on `develop` based on 2970c5c. Scope: `test/wsdl-interop/EXECUTION.md` (P9g-01 entry); the Qore handoff `/tmp/qore-aot-cleanup-slots-stack.md` is outside the repository.
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Root-caused to the Qore AOT compiler with IR dumps and frame measurements; no module workaround (recursion, test depth and stack limit unchanged). The failure stays open. |
...
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Per-level native stack cost measured (88.6 KB) and attributed (71.8 KB cleanup_slots, 7.5 KB used). |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | N/A | No code change. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | EXECUTION.md records the cause, excluded hypotheses and the handoff location. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | Reproduced with the current AOT WSDL.qmod (STACK-LIMIT-EXCEEDED at ~level 84); cited Qore source lines checked; -O3 with no optnone threshold changes frames by only 2-4%. |
