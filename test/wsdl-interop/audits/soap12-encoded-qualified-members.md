# SOAP 1.2 encoded members of qualified schemas audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` on `develop`. Scope: the RPC part decoding in `WSMessage` (`qlib/WSDL.qm`), which no longer checks and strips the members of complex-typed and element parts before their type does; the WSDL v0.5.8 release note; `test/soap12-encoding.qtest` ("qualified struct members decode with their namespace names"); `EXECUTION.md`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | WSDL v0.5.8 release notes describe the fixed decoding of qualified SOAP 1.2 encoded struct members. |
...
| 9 | Copyright 2026 on all new files | Pass | This record carries the 2026 copyright; no new source files. |
...
| 13 | `%modern` directive present | Pass | soap12-encoding.qtest uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | soap12-encoding.qtest keeps its executable bit. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | soap12-encoding.qtest keeps its module path setup. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | No new module dependencies. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | The root cause (a second namespace check after prefix removal) is removed; the remaining check runs once, before the prefixes are removed. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | No new resources; decoding errors raise SOAP-DESERIALIZATION-ERROR as before. |
...
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | No type changes. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | One check per struct instead of two. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Foreign-namespace and unqualified members of a qualified schema are still rejected; both are asserted. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Release note and code comments; docs-WSDL builds without warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No security impact. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | All qtests and all 498 Python tests pass in the developer's normal environment; soap12-encoding 11 cases / 146 assertions; the SOAP 1.2 encoding, RPC, collection and fault suites, the SOAP 1.1 Note examples and the encoded array and reference suites pass; the six binding styles of the P9b workload round-trip. |
