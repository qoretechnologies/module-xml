# P9a-05 WS-Addressing handler audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to P9a-05 on `develop` based on 5d19e71. Scope: WS-Addressing processing, checks, dispatch, replies and faults in `qlib/SoapHandler.qm`; `WsAddressingHelper::endpointPolicy()` and the `all_headers` member of `SOAP-MUST-UNDERSTAND` in `qlib/WSDL.qm`; `test/soap-addressing-handler.qtest`; the WCF case in `test/SoapHandler.qtest`; `design/soap-addressing.md`; and `EXECUTION.md`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | SoapHandler 0.3.4 release notes describe the WS-Addressing processing and the changed default for mandatory WS-Addressing headers; the new soaphandler_addressing section documents it with an example. |
...
| 9 | Copyright 2026 on all new files | Pass | test/soap-addressing-handler.qtest carries the 2026 copyright. |
...
| 12 | `ns=Qore::XX` matches the QoreNamespace constructor path | N/A | No C++ or QPP change; the changes are Qore module code in qlib/SoapHandler.qm and qlib/WSDL.qm, tests and documentation. |
| 13 | `%modern` directive present | Pass | test/soap-addressing-handler.qtest uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/soap-addressing-handler.qtest is executable. |
...
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | QUnit, xml and HttpServer are delivered with Qore; the in-repository modules use relative %requires. |
| 17 | No filesystem operations (fopen, open, creat, unlink, remove, rename, mkdir, rmdir, stat, chmod) without sandbox checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/SoapHandler.qm and qlib/WSDL.qm, tests and documentation. |
| 18 | No network operations (connect, bind, socket, getaddrinfo, gethostbyname) without sandbox checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/SoapHandler.qm and qlib/WSDL.qm, tests and documentation. |
| 19 | If filesystem/network ops exist, verify `QoreSandboxManagerHelper` usage | N/A | No C++ or QPP change; the changes are Qore module code in qlib/SoapHandler.qm and qlib/WSDL.qm, tests and documentation. |
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | The handler opens no connections: every reply and fault is sent on the HTTP response, and non-anonymous response endpoints are rejected, so no request can make the handler contact an address. |
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/SoapHandler.qm and qlib/WSDL.qm, tests and documentation. |
| 22 | Uses `qore_check_cancel()` (NOT deprecated `qore_check_io_interrupt()`) | N/A | No C++ or QPP change; the changes are Qore module code in qlib/SoapHandler.qm and qlib/WSDL.qm, tests and documentation. |
| 23 | Check frequency: every 100 iterations for tight loops, every 10 for expensive iterations | N/A | No C++ or QPP change; the changes are Qore module code in qlib/SoapHandler.qm and qlib/WSDL.qm, tests and documentation. |
| 24 | No blocking operations without cancellation support | N/A | No C++ or QPP change; the changes are Qore module code in qlib/SoapHandler.qm and qlib/WSDL.qm, tests and documentation. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | No stubs: non-anonymous response endpoints are rejected with the WS-Addressing fault that WS-I R1146 allows until the approved, opt-in authorizer of P9a-06; this is documented and tested. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | The handler snapshots its node and setting under the read lock; WsaOutputScope restores on every path; checks run before the callback, so rejected requests have no application side effects. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | setAddressing() and registration update the addressing state and dispatch maps under the existing write lock; request handling reads them under the read lock. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Policies, properties and faults use the typed WsAddressingPolicy, WsaMessageAddressing and WsaFault hashdecls. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Policies and input actions are computed once at registration; request handling adds one header parse and constant-time map lookups. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Every WS-Addressing violation produces the predefined fault with the specified HTTP status; configuration conflicts raise SOAP-HANDLER-ERROR; all are asserted. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | setAddressing(), getAddressing(), cx.addressing and the SOAP-MUST-UNDERSTAND all_headers member are documented; design/soap-addressing.md gains the handler section; docs build without warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | Per the approved default, the handler never contacts sender-chosen addresses (WS-Addressing SOAP Binding section 7); reference parameters are only echoed for anonymous endpoints on the same HTTP response. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | soap-addressing-handler.qtest: 7 cases, 144 assertions, SOAP 1.1 and 1.2, through SoapClient and raw HTTP; SoapHandler.qtest covers the WCF shape under the new default and with WS-Addressing disabled; the related suites pass. The full suite shows no new failures. |
