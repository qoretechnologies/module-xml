# P8-05d attachment binary fidelity audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to P8-05d. Scope: the new `test/soap-attachment-fidelity.qtest` and `test/attachment-fidelity.wsdl`; the fidelity sizes in `test/wsdl-interop/swa-peer/` (SwAPeer.java, qore-peer.qr, README.md) and `test_swa_parts.py`; the 1 MiB MTOM size in `test/wsdl-interop/mtom-peer/`; and the binary fidelity section of `design/wsdl-multipart-input.md`. No module source changed.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | N/A | No module behavior changed; P8-05d adds tests, peers and documentation of the existing behavior. |
...
| 6 | `%modern` in `.qm` file — no redundant `%new-style`, `%require-types`, `%strict-args`, `%enable-all-warnings` | N/A | No module source changed. |
...
| 9 | Copyright 2026 on all new files | Pass | test/soap-attachment-fidelity.qtest, test/attachment-fidelity.wsdl and this record carry the 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | test/soap-attachment-fidelity.qtest uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/soap-attachment-fidelity.qtest is executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | test/soap-attachment-fidelity.qtest uses %prepend-module-path before its relative %requires. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | The P8-05 binary-fidelity requirement is covered for every carrier, size class and framing edge; no module change was needed, and none was made. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Test and peer code only; peers keep their existing bounded start/stop handling and assert on every payload. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | The HTTP test uses per-call clients and a handler without shared state. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed payload helpers (binary) and typed test values. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Payloads are built from repeated units rather than per-octet formatting; the MTOM peer pattern was converted to a unit-based form proven equal to the previous one. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | invalidFraming() checks every rejected framing error class with its message. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | design/wsdl-multipart-input.md gains a binary fidelity section; the SwA and MTOM peer READMEs describe the new sizes; docs build with no warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No credentials, format strings or I/O beyond local test servers. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | soap-attachment-fidelity.qtest (6 cases, 250+ assertions) and the live CXF exchanges (SwA/swaRef to 4 MiB, MTOM to 1 MiB) pass in both directions; payload generators are cross-checked by the peers themselves. |
