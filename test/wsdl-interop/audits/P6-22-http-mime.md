# P6-22 HTTP MIME audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.codex/skills/audit-changes/SKILL.md` to the three module fixes,
regression, release notes and durable design. No C++ or DataProvider changes.
...
| 9 | Copyright 2026 on all new files | Pass | The new regression and design document carry 2026 copyright; modified modules already carry 2026. |
...
| 14 | Executable permission set (`chmod +x`) | Pass | test/wsdl-mimexml-parts.qtest is executable. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Three reproduced causes are fixed directly: reading the input description before its output metadata exists, forcing SOAP versions for HTTP bindings, and recovering SOAP-shaped HTTP errors for non-SOAP bindings. No fallback or suppression was added. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Automatic Qore lifetime handling and existing exception propagation are preserved. The server stops on exit even after assertion or request failure. HTTP errors rethrow their original exception. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Selection reads immutable binding metadata; no shared mutable state is introduced. The callback communicates through Queue. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | The handler uses *bool for the optional version; the regression callback is code<auto(hash<auto>, auto)>; descriptors retain their existing typed classes. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | The changes add constant-time binding selection checks and no loops or copies. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Unknown explicit input/output parts, omitted multiple-part selections, omitted single-part compatibility, source/saved services and detached operations, real success and HTTP failure are tested. Existing SOAP fault/version suites pass. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Release notes in all three modules and durable design/wsdl-http-mime.md describe the implemented behavior with a part-selection example. No public signatures changed; no DGC design changed. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No credentials, raw allocation, new I/O entry point or user-controlled format string. The selected binding limits fault recovery; errors cannot become successful MIME values. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | Nine affected suites pass: 86 cases / 2,765 assertions, including exact wire maps and a real HTTP server. The new regression independently specifies input right/output left values and retains HTTP-CLIENT-RECEIVE-ERROR for the failing request. Existing SOAP 1.1/1.2 fault/version tests protect the other branch. |
