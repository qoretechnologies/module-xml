# P6-26 HTTP MIME XML value audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.codex/skills/audit-changes/SKILL.md` to the HTTP MIME
XML codec, protocol-specific retained-XML checks, regressions and documentation.
...
| 9 | Copyright 2026 on all new files | Pass | New Qore regression and modified module/design documents carry copyright 2026. |
...
| 14 | Executable permission set (`chmod +x`) | Pass | All changed Qore tests are executable. |
...
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | QUnit and HttpServer are core dependencies; xml is delivered by this repository. Repository WSDL/SoapClient/SoapHandler requirements use relative paths. |
...
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | No production I/O change. The integration test uses a bounded loopback HTTP client and on_exit server cleanup. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | MIME XML previously used RPC value serialization without output namespace bindings and a raw-value decoder without root namespace matching. It now uses the document codec with an isolated namespace context and exact selected-part identity. SOAP-only retained-XML restrictions are applied explicitly by protocol. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Per-call namespace and QName scopes unwind on exceptions. No shared state is modified, and the test server is stopped on exit. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Each MIME serialization copies the namespace registry; the focused test compares serialized namespace state before and after calls. No new mutable shared state. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Document/content maps use hash<auto>, selected names use typed strings, and the HTTP callback has code<auto(hash<auto>, auto)> type. Primitive content is wrapped in a heterogeneous map before attributes are added. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | One selected document part is traversed with the existing schema codecs. Namespace copying is once per call; no repeated scans or pairwise operations were introduced. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Tests cover qualified roots/children, prefix/default rebinding, QName values, nil, retained lexical XML, processing instructions, encoding/formatting, nonrepresentable inherited context and DOCTYPE rejection, missing/extra/repeated/wrong roots and invalid content, in source/saved/detached and HTTP paths. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Release notes and durable HTTP MIME design describe the implemented codec and examples. The document codec describes its protocol restriction option. PLAN/EXECUTION record the approved deferred XML/JMS scope and Qore #5454. No DGC changes. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | QName matching prevents wrong-namespace acceptance; namespace output uses existing validated helpers. SOAP processing-instruction rejection remains tested. No new capabilities, unmanaged buffers, credentials or dynamic format strings. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | The affected gate passes 14 suites / 169 cases / 3989 reported assertions. Independently written XML uses alternate prefixes/defaults; the real HTTP path and saved graphs preserve expanded names and native values. All 16 corpus commands meet expected outcomes. WSDL 1.1 section 5.6 defines the selected schema root. |
