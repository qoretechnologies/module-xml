# P6-15 explicit body-part selection audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.codex/skills/audit-changes/SKILL.md` to explicit body-part selection,
RPC decoding, fault-detail selection, fixtures, Qore and independent oracle tests,
docs and evidence. Reviewed
...
| 9 | Copyright 2026 on all new files | Pass | New Qore/Python/Java/fixture/design/evidence files carry 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | The new Qore test declares %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | wsdl-body-parts.qtest is executable (0755). |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | The test prepends local qlib before relative in-repo module requires. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | No new external Qore dependency; core QUnit/HttpServer use hard requires. Independent tests reuse pinned WSDL4J/Xerces and existing lxml. |
...
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | No new production I/O. Tests read a repository fixture and use loopback HTTP with Queue completion, bounded deadlines and on_exit cleanup. Oracle subprocesses have deadlines and managed temporary directories. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Fixes lost empty selections, ignored RPC subsets and wrong-message fault filtering at their causes. XML whitespace uses the existing schema normalizer. Oracle tokenizer disagreements are independently adjudicated, without copying its defect or skipping module regressions. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Construction validation discards local incomplete descriptions on error. RPC decoding mutates local copies and returns only after successful conversion; no shared operation state changes. HTTP test cleanup runs on every exit. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Part lists are initialized during binding construction, before shared use. Per-call RPC maps also detect repeated names; no mutable global state or shared per-call caches are added. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Body parts use *list<string>; RPC selection uses *softlist<string> and an optional bool policy. HTTP callback has code<auto(hash<auto>, auto)>. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Whitespace normalization, duplicate checking and binding reference validation are linear in the selected list. Hash membership avoids repeated full message scans; RPC duplicate validation reuses the local result map. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Omitted/empty/single/multiple selections, XML whitespace, explicit order, unknown/duplicate names, extra and missing values, sole-argument inference, public partial decoding, saved graphs, headers and fault detail are tested. Audit found and fixed unknown public RPC parts producing a Qore type error. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | The constructor and RPC API document parameters, errors, partial/strict behavior and an example. Release notes and durable body-part design are updated. WSDL Doxygen and astparser checks pass. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No unmanaged buffers, new production I/O, secrets or variable format strings. Name membership is checked before dereferencing part metadata; oracle artifacts are checksum-verified. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | Focused 9 cases / 467 assertions pass. WSDL4J observes explicit lists; pinned Xerces confirms XML list value normalization, with WSDL4J tokenizer limitations recorded separately. The complete final-source regression gate and six corpus report comparisons pass. |

All 62 checks: **18 Pass / 44 N/A / zero Fail**.

See [validation](../P6-15-validation.json) and [evidence](../body-parts-evidence.md).
