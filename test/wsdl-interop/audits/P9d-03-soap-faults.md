# P9d-03 generated SOAP faults audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` on `develop` based on 32d49ef. Scope: `test/wsdl-interop/test_soap_fault_generation.py`, `fault-info.qr`, `PLAN.md`, `EXECUTION.md`.
...
| 9 | Copyright 2026 on all new files | Pass | test_soap_fault_generation.py, fault-info.qr and this record carry the 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | fault-info.qr uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test_soap_fault_generation.py and fault-info.qr are executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | The worker uses %prepend-module-path before its relative %requires. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | The worker uses the %try-module pattern for json; xml and WSDL are part of this module. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Adds generated SOAP fault coverage required by P9, with expectations from the pinned W3C envelope schemas and lxml's QName resolution; the one schema/protocol difference is the P7-05 adjudication, asserted separately. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | N/A | No module code changed. |
...
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed worker locals; QName values are reported as expanded names. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | One worker process; the whole test runs in about 4 s. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Twelve fault-grammar mutations, 15 each, must be rejected by the pinned schema and by the module with SOAP-DESERIALIZATION-ERROR; a removed xml:lang check was detected. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | The test docstring describes the generator, reference and mutations; PLAN.md and EXECUTION.md record the coverage. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | 5 tests pass: 300 schema-valid faults read as lxml resolves them, 180 invalid faults rejected by schema and module, 15 schema-only differences read correctly; the pinned digest is identical on CPython 3.12 and 3.14. |
