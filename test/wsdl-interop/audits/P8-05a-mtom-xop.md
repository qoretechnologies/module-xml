# P8-05a MTOM/XOP reconstruction and packaging audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to the uncommitted P8-05a changes on main `develop`
based on 4a0607e. Scope: `qlib/WSDL.qm` (substXopInclude rewrite, contentIdFromUri, packageMtom XML type and
start-info, multipart start-info check, release notes), `test/soap-mtom.qtest`,
`test/wsdl-interop/test_attachment_sources.py`, the pinned XOP 1.0, SOAP 1.2 MTOM and RFC 2392 sources and their
`sources.json` entries, `.gitattributes`, `design/wsdl-multipart-input.md`, and PLAN.md and EXECUTION.md.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | WSDL v0.5.8 release notes state that MTOM input now works and describe the XOP rules, percent-decoded cid: references, start-info and the SOAP 1.2 packaging type. |
...
| 9 | Copyright 2026 on all new files | Pass | test/soap-mtom.qtest, test/wsdl-interop/test_attachment_sources.py and this record carry the 2026 copyright; the pinned W3C and IETF sources are verbatim and not relicensed. |
...
| 12 | `ns=Qore::XX` matches the QoreNamespace constructor path | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and pinned sources. |
| 13 | `%modern` directive present | Pass | test/soap-mtom.qtest uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/soap-mtom.qtest and test_attachment_sources.py are executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | test/soap-mtom.qtest uses %prepend-module-path before the relative %requires of WSDL.qm. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | No external module: QUnit is delivered with Qore and xml is this repository's module. |
| 17 | No filesystem operations (fopen, open, creat, unlink, remove, rename, mkdir, rmdir, stat, chmod) without sandbox checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and pinned sources. |
| 18 | No network operations (connect, bind, socket, getaddrinfo, gethostbyname) without sandbox checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and pinned sources. |
| 19 | If filesystem/network ops exist, verify `QoreSandboxManagerHelper` usage | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and pinned sources. |
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | No I/O added to the module; the suite builds packages in memory, and test_attachment_sources.py reads the pinned files. |
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and pinned sources. |
| 22 | Uses `qore_check_cancel()` (NOT deprecated `qore_check_io_interrupt()`) | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and pinned sources. |
| 23 | Check frequency: every 100 iterations for tight loops, every 10 for expensive iterations | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and pinned sources. |
| 24 | No blocking operations without cancellation support | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and pinned sources. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Complete for XOP reconstruction and packaging. MTOM output, swaRef and SwA references are the planned P8-05b and P8-05c increments, not stubs; the unreferenced-part and extension-ignoring behavior is specified by XOP 1.0 section 2.1 and SOAP 1.2 MTOM section 4.3.1. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | substXopInclude() validates the whole tree through a local used-part set before the caller uses it; failures raise SOAP-MESSAGE-ERROR and leave no shared state. contentIdFromUri() rejects malformed percent-encodings before decoding. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | All state is per call: the namespace scopes and used-part set are locals, and the helpers are static. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed hash<string, bool> used-part set, typed scope hashes, and a typed xml_type parameter with the SOAP 1.1 default so existing callers are unchanged. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Reconstruction is one pass over the tree with a hash lookup per reference; percent-decoding is linear. The 50-part and 1 MiB cases run in the suite, and the full suite ran at its previous cost (393 parallel targets in 873 s). |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Each rejection names the reference or part and cites its section: non-sole child, unqualified or XOP-namespace attributes and children, missing href, non-cid URI, malformed percent-encoding, invalid Content-ID, missing part, duplicate reference, and a conflicting start-info. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | design/wsdl-multipart-input.md (MTOM/XOP reconstruction), Doxygen for substXopInclude, contentIdFromUri and packageMtom, the release notes, and the PLAN.md and EXECUTION.md entries. Docs build with no warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No credentials; constant format strings; Content-IDs with control characters or angle brackets are rejected, and no reference can select an unidentified part. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | Checked against XOP 1.0 sections 2, 3.2 and 4.1, SOAP 1.2 MTOM sections 3.2 and 4.3.1 and RFC 2392, pinned with digests; CXF's percent-encoded references are covered. The suite fails against the previous module, and the full suite on one runtime fails only on the two core-blocked IEEE gates. |
