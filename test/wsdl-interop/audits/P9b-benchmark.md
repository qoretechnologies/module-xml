...
Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to P9b on `develop` based on 96d861e. Scope: `test/wsdl-interop/benchmark/` (`bench.qr`, `benchmark.py`, `README.md`, `reference.json`, `workloads/list-values.tar.xz`, `workloads/binding-styles.wsdl`), `test/wsdl-interop/test_benchmark.py`, `PLAN.md` and `EXECUTION.md`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | N/A | No module behavior change; the benchmark is documented in benchmark/README.md, PLAN.md and EXECUTION.md. |
...
| 6 | `%modern` in `.qm` file — no redundant `%new-style`, `%require-types`, `%strict-args`, `%enable-all-warnings` | Pass | bench.qr uses %modern. |
...
| 9 | Copyright 2026 on all new files | Pass | bench.qr, benchmark.py, test_benchmark.py, README.md, binding-styles.wsdl and this record carry the 2026 copyright; reference.json has a copyright key; the list-values archive holds generated WSDLs captured from the test generator. |
...
| 12 | `ns=Qore::XX` matches the QoreNamespace constructor path | N/A | No C++ or QPP change; P9b adds a benchmark (Qore driver, Python harness, pinned workloads, reference) and its gate. |
| 13 | `%modern` directive present | Pass | bench.qr uses %modern (it is a script, run by benchmark.py, not a qtest). |
| 14 | Executable permission set (`chmod +x`) | Pass | bench.qr, benchmark.py and test_benchmark.py are executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | bench.qr uses %prepend-module-path before its relative %requires of WSDL.qm. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | json comes from another repository and uses the %try-module pattern of the existing worker; xml is this repository's module. |
| 17 | No filesystem operations (fopen, open, creat, unlink, remove, rename, mkdir, rmdir, stat, chmod) without sandbox checks | N/A | No C++ or QPP change; P9b adds a benchmark (Qore driver, Python harness, pinned workloads, reference) and its gate. |
| 18 | No network operations (connect, bind, socket, getaddrinfo, gethostbyname) without sandbox checks | N/A | No C++ or QPP change; P9b adds a benchmark (Qore driver, Python harness, pinned workloads, reference) and its gate. |
| 19 | If filesystem/network ops exist, verify `QoreSandboxManagerHelper` usage | N/A | No C++ or QPP change; P9b adds a benchmark (Qore driver, Python harness, pinned workloads, reference) and its gate. |
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | The driver only reads the workload files; benchmark.py unpacks the pinned archive into a temporary directory with tarfile's data filter. |
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | N/A | No C++ or QPP change; P9b adds a benchmark (Qore driver, Python harness, pinned workloads, reference) and its gate. |
| 22 | Uses `qore_check_cancel()` (NOT deprecated `qore_check_io_interrupt()`) | N/A | No C++ or QPP change; P9b adds a benchmark (Qore driver, Python harness, pinned workloads, reference) and its gate. |
| 23 | Check frequency: every 100 iterations for tight loops, every 10 for expensive iterations | N/A | No C++ or QPP change; P9b adds a benchmark (Qore driver, Python harness, pinned workloads, reference) and its gate. |
| 24 | No blocking operations without cancellation support | Pass | Driver runs have a 30-minute hang guard; no polling; the gate makes no timing assertions. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | No stubs: every planned phase is measured for both workloads, against a recorded reference, with byte-identical output checks. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Temporary directories are removed by context managers; driver failures raise with their output. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | N/A | Single-threaded harness. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed Qore driver (%modern); Python uses explicit structures. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | The benchmark measures; medians over repetitions limit noise, and the load average is recorded and checked. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Changed inputs, changed outputs, missing phases, non-Release builds and busy machines are refused or reported; the comparison rules are unit-tested. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | benchmark/README.md documents workloads, phases, usage, exit statuses and the environment rules; PLAN.md and EXECUTION.md record P9b. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | test_benchmark.py passes (5 tests) against the recorded reference; benchmark.py --record ran on the Release build (5 repetitions); the rest of the suite passed at 96d861e (the benchmark adds only files). |
