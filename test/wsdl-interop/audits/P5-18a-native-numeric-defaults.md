# P5-18a native canonical numeric declarations audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied the complete audit-changes skill and all five referenced design guides on
2026-09-13 to the final changes based on `ace866d`. The guide hashes remain equal
to the versions already read for P5-17b. Native C is reviewed for ownership,
bounds, errors and threading. See [evidence](../native-numeric-defaults-evidence.md)
and [inventory](../P5-18a-validation.json).
...
| 9 | Copyright 2026 on all new files | Pass | All new authored code, tests and design/evidence carry 2026 copyright. JSON is test data; third-party source notices remain intact. |
...
| 13 | `%modern` directive present | Pass | xml-numeric-defaults.qtest uses %modern; Python runs as a separate unittest entry point. |
| 14 | Executable permission set (`chmod +x`) | Pass | Both new test entry points are executable (0755). |
...
| 24 | No blocking operations without cancellation support | Pass | No blocking operation introduced. Existing native cancellation and reader cleanup suites pass in the 150-suite gate. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Canonical declaration checks fix the omitted XSD 1.0 requirement. No fallback or PSVI workaround is introduced; other default requirements remain explicit P5 work. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Temporary buffer and per-item strings are freed on all paths; declaration values retain their original owner. All 2394 injected faults recover, and native/Qore Valgrind report zero errors or lost memory. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | All new mutable state belongs to one checker invocation. Shared schema constraints are read without mutation; no global mutable production state. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Native pointers and enum-valued test paths follow libxml2 interfaces. Qore regressions use typed collections/callbacks. No new public Qore result API. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Two linear passes over selected items and one lexical scan; growable xmlBuf avoids repeated whole-string concatenation. Exact large numeric and 180-item growth cases pass. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Original and canonical validity are both enforced; all 194 invalid declarations reject as XSD-SYNTAX-ERROR. Required allocation failures propagate; OOM while reporting an already rejected pattern retains rejection plus NO_MEMORY context. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Native design includes a realistic numeric pattern example; README, release notes, plan, execution record and reproducible evidence are updated. Affected Doxygen builds without warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No network/filesystem production operation, user-controlled format string or credential. C probe buffers check snprintf bounds; temporary buffer growth propagates errors. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | 396 pinned-Xerces schemas agree, 987 native assertions pass, all 150 Qore suites and 53 provider checks pass. Both complete corpus modes match parent results at every recorded case/stage. |
