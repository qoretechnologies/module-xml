# qore#5464 named group alternatives audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to the port of the 2.x optional and named model group tests (`d236358`, note `/tmp/module-xml-optional-groups-develop-note.md`) and the defect they exposed on `develop`. Scope: `XsdComplexType` field projection in `qlib/WSDL.qm` with the release note, `design/wsdl-particles.md`, `test/optional-choice.wsdl` and `test/soap.qtest`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | The WSDL v0.5.8 release notes describe the corrected field view and link issue 5464. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Root cause: finalize() merged every referenced group's elements into the type's elementmap, wherever the reference stood, so a group alternative of a choice became sequence fields. The field view is now projected from the resolved particle (projectParticle()/projectModelGroupMember()), with no special case. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | The re-projection restores every projected member on error (on_error), and finalize() already restores the particle, attributes and content flags. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Projection runs once in finalize(), before the type is published. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | switch over the XsdParticleKind enum; typed ChoiceInfo and XsdElement maps. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Each particle is projected at most twice (once inside optional groups, once where required) and each choice alternative's elements are collected once, so a shared group graph is walked linearly. The first version expanded every path and hung wsdl-mixed-base's sharedGraph() case (32 nested choices of two references each, 2^32 paths); the full suite caught it, and the four affected suites run at their previous times again. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | An unresolved group reference still raises WSDL-ERROR; a partial optional group and a missing required group member are rejected in both directions. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | design/wsdl-particles.md describes the projection; release note added. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | Ported the 2.x d236358 named-group contract and modelGroupTest (errors mapped to develop's particle validator), plus the optional-group decode and round trip requested for develop and example/provider assertions; against the previous module the group-alt decode reports an extra "g1" field. |
