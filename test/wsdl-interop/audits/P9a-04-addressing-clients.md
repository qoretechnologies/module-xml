# P9a-04 WS-Addressing clients audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to P9a-04 on `develop` based on 5c0aca8. Scope: `SoapNodeOptions.addressing`, `SoapProcessingNode::getOptions()`, `WsAddressingHelper::clientRequest()` and `clientResponse()` in `qlib/WSDL.qm`; the WS-Addressing options and call handling in `qlib/SoapClient.qm` and `qlib/SoapClientIo`; `test/soap-addressing-client.qtest`; the node case in `test/soap-addressing-messages.qtest`; `design/soap-addressing.md`; and `EXECUTION.md`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | Release notes: SoapClient v1.0.4, SoapClientIo v1.0 and WSDL v0.5.8 gain WS-Addressing entries; SoapClient documents the behavior in the new soapclient_addressing section. |
...
| 9 | Copyright 2026 on all new files | Pass | test/soap-addressing-client.qtest carries the 2026 copyright. |
...
| 12 | `ns=Qore::XX` matches the QoreNamespace constructor path | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, qlib/SoapClient.qm and qlib/SoapClientIo, tests and documentation. |
| 13 | `%modern` directive present | Pass | test/soap-addressing-client.qtest uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/soap-addressing-client.qtest is executable. |
...
| 17 | No filesystem operations (fopen, open, creat, unlink, remove, rename, mkdir, rmdir, stat, chmod) without sandbox checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, qlib/SoapClient.qm and qlib/SoapClientIo, tests and documentation. |
| 18 | No network operations (connect, bind, socket, getaddrinfo, gethostbyname) without sandbox checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, qlib/SoapClient.qm and qlib/SoapClientIo, tests and documentation. |
| 19 | If filesystem/network ops exist, verify `QoreSandboxManagerHelper` usage | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, qlib/SoapClient.qm and qlib/SoapClientIo, tests and documentation. |
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | The clients use their existing HTTPClient and async connection manager; no new I/O path is added. The tests use a local HttpServer on an ephemeral loopback port. |
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, qlib/SoapClient.qm and qlib/SoapClientIo, tests and documentation. |
| 22 | Uses `qore_check_cancel()` (NOT deprecated `qore_check_io_interrupt()`) | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, qlib/SoapClient.qm and qlib/SoapClientIo, tests and documentation. |
| 23 | Check frequency: every 100 iterations for tight loops, every 10 for expensive iterations | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, qlib/SoapClient.qm and qlib/SoapClientIo, tests and documentation. |
| 24 | No blocking operations without cancellation support | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, qlib/SoapClient.qm and qlib/SoapClientIo, tests and documentation. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | No stubs: the request-response restriction to anonymous response endpoints is an explicit, documented and tested rejection until the planned decoupled responses of P9a-06. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | WsaOutputScope restores on every path; response node derivation creates a new immutable node per call; the fault path keeps the original exception. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Per-call state only; the client options are immutable after construction and nodes stay immutable. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed WsaMessageAddressing values throughout; option types are validated; node options are derived as typed hashes. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | One header parse per response; no additional round trips. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Invalid options raise SOAP-CLIENT-ERROR or SOAP-CLIENT-IO-ERROR, policy violations WSA-CLIENT-ERROR, invalid properties WSA-SERIALIZATION-ERROR, response mismatches WSA-RESPONSE-ERROR; all are asserted. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Constructor and call options, the info key and the new node option and getOptions() are documented with examples; design/soap-addressing.md gains the node and client sections; docs build without warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No credentials or format strings; message IDs are random; reference parameters from the WSDL are validated before sending. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | soap-addressing-client.qtest: 7 cases, 131 assertions, both clients, SOAP 1.1 and 1.2; the node capability case in soap-addressing-messages.qtest; the related SoapClient, soap-client-io, SoapHandler, soap-envelope, soap-actions, soap-http-faults and soap-oneway suites pass. The full suite shows no new failures. |
