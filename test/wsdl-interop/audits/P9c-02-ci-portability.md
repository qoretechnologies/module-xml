# P9c-02 CI portability findings audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` on `develop` based on c9b6233. Scope: `src/XsdFloat.cpp` (`parseMagnitude()`, `parseFinite()`); `test/xsd-float.qtest`; `test/wsdl-interop/test_soap_duplex.py`; `docs/mainpage.doxygen.tmpl`; `EXECUTION.md`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | docs/mainpage.doxygen.tmpl extends the unreleased convert_xsd_float() note: decimal text is rounded by magnitude and then signed. |
...
| 6 | `%modern` in `.qm` file — no redundant `%new-style`, `%require-types`, `%strict-args`, `%enable-all-warnings` | N/A | No .qm change. |
| 7 | No parse directives (`%requires`, `%modern`, `%new-style`) in separated `.qc` files (check OUTSIDE of `@code` blocks only) | N/A | No .qc change. |
| 8 | No `%include` usage (deprecated for modules) | Pass | No %include usage. |
| 9 | Copyright 2026 on all new files | Pass | No new source file; this record carries the 2026 copyright. |
...
| 12 | `ns=Qore::XX` matches the QoreNamespace constructor path | N/A | No QPP class added; src/XsdFloat.cpp is internal C++. |
| 13 | `%modern` directive present | Pass | test/xsd-float.qtest keeps %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/xsd-float.qtest remains executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | test/xsd-float.qtest keeps %prepend-module-path. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | QUnit and xml are delivered with Qore and this module. |
| 17 | No filesystem operations (fopen, open, creat, unlink, remove, rename, mkdir, rmdir, stat, chmod) without sandbox checks | Pass | No filesystem operation added to C++. |
| 18 | No network operations (connect, bind, socket, getaddrinfo, gethostbyname) without sandbox checks | Pass | No network operation added to C++. |
| 19 | If filesystem/network ops exist, verify `QoreSandboxManagerHelper` usage | N/A | No filesystem or network operation. |
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | N/A | No Qore module code changed; the Python peer uses loopback sockets only. |
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | Pass | No loop added; the existing lexical scans keep their qore_check_cancel() checks every 100 characters, and the conversion keeps its check. |
| 22 | Uses `qore_check_cancel()` (NOT deprecated `qore_check_io_interrupt()`) | Pass | qore_check_cancel() is used, unchanged. |
| 23 | Check frequency: every 100 iterations for tight loops, every 10 for expensive iterations | Pass | Unchanged check frequency. |
| 24 | No blocking operations without cancellation support | Pass | No blocking operation added; the peer's handlers keep their 120 s socket timeout and release deadline, so joining them is bounded. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Root-caused in the CI images with podman: musl's strtof/strtod round six negative subnormal-tie values incorrectly (bit-for-bit comparison of 1,248 lexicals with the exact reference; glibc 0 mismatches); the test peer discarded a valid pipelined request and read records before joining its handlers. The libxml2 oracle version issue is recorded as open, not worked around. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | parseFinite() returns after checking xsink when parseMagnitude() raises; XsdFloatEnvironment still restores the IEEE environment on every path; no allocation added. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | No shared state added in C++; the peer's handler records are joined before they are read. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed template parameters and const bool; no casts added. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | One extra character comparison per conversion. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | The lexical is validated before conversion as before, so the stripped sign is always followed by a valid unsigned lexical (asserted with assert(start < end)). |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Comments in src/XsdFloat.cpp explain the symmetric rounding and the musl behavior; EXECUTION.md records the findings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No format strings built from input; no credentials. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | xsd-float.qtest (615 assertions), xsd-float-canonical, wsdl-ieee-facets, wsdl-ieee-constraints(-http) and six Python IEEE modules (22 tests) pass locally; xsd-float.qtest, test_ieee_conversion and test_ieee_scalars pass in the Alpine image; test_soap_duplex passes locally and in the Alpine image. Valgrind: no definite leaks; the one error is Qore's known report, reproduced without this module. |
