# P9b-02 SOAP message decoding audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` on `develop` based on d23004d. Scope: `XsdBase::getAttributeNamespaces()`, `validateAttributeNamespace()` with `XsdBase::ValidBindings`, `getExpandedAttributes()`, `expandElementNamespaces()` and `expandElementNamespacesIntern()`, and the `XsdQNameNamespaceScope` constructor in `qlib/WSDL.qm`; `test/wsdl-namespace-expansion-cache.qtest`; `design/wsdl-qname-values.md`; `EXECUTION.md`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | WSDL v0.5.8 release notes describe the decoding change and its effect. |
...
| 9 | Copyright 2026 on all new files | Pass | test/wsdl-namespace-expansion-cache.qtest and this record carry the 2026 copyright. |
...
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | QUnit and xml are delivered with Qore and this module; WSDL is required by relative path. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Root-caused with a caller-edge sampling profile: repeated derivation of per-element namespace information. The caches are exact: element names are cached per document, bindings only when valid, and every invalid input is still rejected. A pairIterator() variant that slowed RPC/encoded decoding was measured and replaced. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | No C++ change. The memo is updated only after validation succeeds, by replacing the whole hash, so an exception leaves it unchanged. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | XsdBase::ValidBindings is read into a local and replaced as a whole on update, so concurrent readers see a consistent hash; tested from four threads. The per-document name cache is a local passed by reference. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed hash<string, bool>, hash<string, list<string>> and hash<string, string> members and locals; the xml binding constant is cast to hash<string, string>. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Decoding a 50-item order: document/literal 305-339 -> 238-242 ms, RPC/literal 313-439 -> 255-262 ms, RPC/encoded unchanged within noise; outputs byte-identical to the P9b reference. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Invalid element names, unbound prefixes and invalid bindings are rejected on every call (asserted repeatedly, after cached valid entries and beyond the memo limit). |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | design/wsdl-qname-values.md documents the name and binding caches; EXECUTION.md records the profile and results. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | Full suite on a snapshot: every test/*.qtest file passes and the Python suite (six shards, ci_suite.py verify) runs 523 tests, all passing. The new test also passes against the P9b-01 module. During the run build-debug was rebuilt with an unrelated XsdFloat.cpp change that gives identical results on glibc. |
