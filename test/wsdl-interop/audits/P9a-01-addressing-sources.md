# P9a-01 WS-Addressing sources and scope audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to P9a-01 on `develop` based on adecc7a. Scope: the seven pinned WS-Addressing and WS-Policy sources in `test/wsdl-interop/normative/` with their `sources.json` entries and `.gitattributes` lines, `test/wsdl-interop/test_addressing_sources.py`, and the P9a entries in `PLAN.md` and `EXECUTION.md`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | N/A | No module behavior changed; release notes follow with the implementing increments. |
...
| 9 | Copyright 2026 on all new files | Pass | test_addressing_sources.py and this record carry the 2026 copyright. The pinned W3C documents are third-party files that keep their own W3C notices, which the test asserts. |
...
| 12 | `ns=Qore::XX` matches the QoreNamespace constructor path | N/A | No module code change; this increment pins sources, adds a source test and records the approved scope. |
| 13 | `%modern` directive present | N/A | No .qtest changed; the new test is Python. |
| 14 | Executable permission set (`chmod +x`) | Pass | test_addressing_sources.py is executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | N/A | No Qore test changed. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | N/A | No Qore test changed. |
| 17 | No filesystem operations (fopen, open, creat, unlink, remove, rename, mkdir, rmdir, stat, chmod) without sandbox checks | N/A | No module code change; this increment pins sources, adds a source test and records the approved scope. |
| 18 | No network operations (connect, bind, socket, getaddrinfo, gethostbyname) without sandbox checks | N/A | No module code change; this increment pins sources, adds a source test and records the approved scope. |
| 19 | If filesystem/network ops exist, verify `QoreSandboxManagerHelper` usage | N/A | No module code change; this increment pins sources, adds a source test and records the approved scope. |
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | No I/O in module code; the sources were retrieved once over verified TLS (--proto =https, explicit CA bundle) and the test reads only the pinned local files. |
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | N/A | No module code change; this increment pins sources, adds a source test and records the approved scope. |
| 22 | Uses `qore_check_cancel()` (NOT deprecated `qore_check_io_interrupt()`) | N/A | No module code change; this increment pins sources, adds a source test and records the approved scope. |
| 23 | Check frequency: every 100 iterations for tight loops, every 10 for expensive iterations | N/A | No module code change; this increment pins sources, adds a source test and records the approved scope. |
| 24 | No blocking operations without cancellation support | N/A | No module code change; this increment pins sources, adds a source test and records the approved scope. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Nothing is stubbed: the increment adds only pinned sources, their test and the plan; the two cited-phrase adjustments (a markup space before a period, FaultDetail defined only by the SOAP Binding text) match the documents instead of editing them. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | N/A | No C++ or module code. |
...
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | The test compares typed sets and exact digests. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | The test parses seven small files once. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Missing or altered sources, missing phrases, wrong namespaces or missing fault codes fail the test. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | PLAN.md records the approved scope, defaults and increments P9a-01 to P9a-08; EXECUTION.md records the sources, the baseline gaps found in the code and the Metadata example 4-8 erratum; sources.json documents provenance and licenses. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | Downloads used verified TLS; no credentials; the plan makes non-anonymous response endpoints opt-in because of the SOAP Binding section 7 warning. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | test_addressing_sources.py: 4 tests pass (digests of all 7 sources, 30 cited phrases, cited documents pinned, schema namespaces/elements/fault codes); test_attachment_sources.py and test_normative.py still pass with the extended sources.json. |
