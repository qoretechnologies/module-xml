# P5-18c native binary constraints audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied every audit-changes checklist item to the exact final changes based on
`8cec041` on 2026-09-13. All five referenced guides were read previously and their
hashes remain unchanged. Native dependency changes are checked for ownership,
error status, bounds, thread safety and cancellation boundaries.
See [evidence](../native-binary-constraints-evidence.md) and [inventory](../P5-18c-validation.json).
...
| 13 | `%modern` directive present | Pass | xml-binary-constraints.qtest uses %modern; the Python oracle test is a separate unittest entry point. |
| 14 | Executable permission set (`chmod +x`) | Pass | The new qtest and Python entry point are executable (0755). |
...
| 24 | No blocking operations without cancellation support | Pass | No blocking operation introduced. Existing native cancellation and reader cleanup suites pass in the final 153-suite gate. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | The binary canonical and lexical omissions are fixed directly. Explicit normalization status resolves the ambiguous-null error path; retaining the first buffer fixes its ownership leak. No fallback or PSVI workaround; remaining WSDL/other canonical families stay assigned to P5. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Both binary temporary values and normalization buffers retain one owner. Private status propagation preserves the public ABI and cleans up before returning. All 151 datatype and 2688 canonical allocation faults recover; native/direct-Qore Valgrind report zero errors or lost memory. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | New production state is local to the normalization/checker invocation. Fixed shared type metadata is read without new mutation; allocator hooks are confined to standalone single-process probes. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Native pointers and status integers follow libxml2 interfaces. No public signature changes; qtests use typed collections/callbacks and exact error categories. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Existing linear normalization algorithms are retained; an already owned normalized buffer avoids a duplicate pass. Binary canonical strings use existing computed values and the linear joining buffer. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | All 96 invalid canonical declarations reject as XSD-SYNTAX-ERROR. Invalid Base64 alphabet/padding cases reject through all native APIs. Allocation failures are internal errors, not lexical failures; complete sweeps cover computed/noncomputed values and rejection recovery. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Design, README, native release notes, plan, execution record, primary specification links and reproduction commands cover binary canonicalization and normalization ownership. Affected native API documentation builds without warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | Strict Base64 input rejects punctuation and non-XML spacing. No new production I/O, credentials or user-controlled diagnostic format. All new C buffer formatting checks bounds; test subprocesses are bounded. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | All 153 final Qore suites and 55 final provider checks pass. Pinned Xerces agrees on 208 schemas and 229 documents; 570 native assertions and complete allocation/Valgrind checks pass. Both corpus modes equal every parent case/stage, with later-phase failures still visible. |
