# P9c-01 Python suite in CI audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` on `develop` based on 33103b2. Scope: `.gitlab-ci.yml`; `test/docker_test/test-ubuntu.sh`, `test-alpine.sh`, `check-i18n.sh` and the new `run-python-suite.sh`; `test/wsdl-interop/ci_suite.py`, `ci-timings.json`, `jvm.py`, `test_ci_suite.py`, `test_jvm.py`; the Java subprocess environments in `axis_interop.py`, `independent.py` and 16 test modules; `test_particle_corpus.py`; `README.md`; `.gitignore`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | N/A | No module change; test/wsdl-interop/README.md documents the CI jobs, requirements and runner. |
| 3 | `qore_user_module()` or `qore_external_user_module()` call in `CMakeLists.txt` | N/A | No module added; the change is to CI and the Python test suite. |
| 4 | Module added to QMOD list in `CMakeLists.txt` | N/A | No module added; the change is to CI and the Python test suite. |
| 5 | `.qm` file has `@section <lowercasemodname>intro` as first doc section — **must be all lowercase** (e.g., `avrodataproviderintro`, not `AvroDataProviderintro`) | N/A | No module added; the change is to CI and the Python test suite. |
...
| 9 | Copyright 2026 on all new files | Pass | ci_suite.py, test_ci_suite.py, jvm.py, test_jvm.py, run-python-suite.sh, ci-timings.json and this record carry the 2026 copyright. |
| 10 | Directory layout: `.qm` inside `qlib/<ModuleName>/` directory (not at `qlib/<ModuleName>.qm` for multi-file modules) | N/A | No module added; the change is to CI and the Python test suite. |
| 11 | No second `.qm` for the same module at `qlib/<ModuleName>.qm` | N/A | No module added; the change is to CI and the Python test suite. |
| 12 | `ns=Qore::XX` matches the QoreNamespace constructor path | N/A | No C++ or QPP change. |
| 13 | `%modern` directive present | N/A | No .qtest file changed; the new tests are Python unittest modules. |
| 14 | Executable permission set (`chmod +x`) | Pass | ci_suite.py, test_ci_suite.py, test_jvm.py and run-python-suite.sh are executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | N/A | No .qtest file changed. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | N/A | No Qore test dependencies changed. |
...
| 24 | No blocking operations without cancellation support | N/A | No blocking operation added; subprocesses keep their existing timeouts. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Root-caused: the JVM launchers print 'Picked up JAVA_TOOL_OPTIONS' on stderr when the CI images set that variable, and the oracles rightly reject unexpected diagnostics; Java children now run without ambient JVM options (jvm.py) rather than tolerating the notice. test_particle_corpus.py extracts the pinned corpus instead of depending on a manual /tmp extraction. No TODOs or stubs. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | No C++ change. Temporary directories use TemporaryDirectory with cleanups; ci_suite.py writes results after each shard and verify creates its output directory. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Shards are separate processes and jobs; jvm.environment() returns a new dict and never changes os.environ. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Shard arguments are parsed and range-checked (parse_shard); timings are validated as non-negative numbers. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Shards are balanced by measured durations (about 854 s each of 5,121 s with six shards); the verify step is linear in the number of tests. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | A missing shard, missing, duplicate, unknown, skipped, failed or misassigned test, an import failure or a module without tests fails verify (test_ci_suite.VerifyTest); malformed timings and shard specifications are rejected. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | README.md 'Continuous integration' section; module docstrings in ci_suite.py and jvm.py; comments in .gitlab-ci.yml and the CI scripts. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No credentials; GitHub statuses keep using the masked GITHUB_ACCESS_TOKEN CI variable; shard tests run as the unprivileged qore user. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | test_ci_suite.py (17 tests) and test_jvm.py (3 tests, including the real java and javac launchers) pass. A local run of four shards with JAVA_TOOL_OPTIONS set, as in the images, ran 523 tests in 187 modules and verify reported all passed; before the jvm.py fix the same run failed 256 tests, all from the launcher notice. CI uses six shards; the assignment only groups the same modules differently (test_ci_suite.AssignmentTest). The .gitlab-ci.yml passes GitLab lint, and check-i18n.sh passes with build and build-debug. |
