# P9h-01 XSD name characters audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` on `develop` based on 4d6a30e. Scope: new `cmake/QoreXmlLibXml2NameEditionFix.cmake`, `cmake/libxml2-name-edition-probe.h`; `cmake/QoreXmlLibXml2.cmake`, `cmake/libxml2-namespace-probe.c`, `Makefile.am`; new `test/xml-name-edition.qtest`; `test/cmake/test_libxml2_provider.py`; new `design/xml-name-characters.md`; `docs/mainpage.doxygen.tmpl` (release note); `test/wsdl-interop/EXECUTION.md`.
...
| 9 | Copyright 2026 on all new files | Pass | All new files carry the 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | xml-name-edition.qtest uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | xml-name-edition.qtest is executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | xml-name-edition.qtest uses %prepend-module-path before %requires. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | Only QUnit and the in-repo xml module are required. |
| 17 | No filesystem operations (fopen, open, creat, unlink, remove, rename, mkdir, rmdir, stat, chmod) without sandbox checks | N/A | No filesystem operation added. |
| 18 | No network operations (connect, bind, socket, getaddrinfo, gethostbyname) without sandbox checks | N/A | No network operation added. |
...
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | N/A | The added scans are bounded by the value length, like the upstream functions they replace. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Root-caused to libxml2 2.15's Fifth Edition scan in xmlValidate*(); the fix restores the Second Edition tables only in the schema processor, as decided by the user (no per-SOAP-version mode). |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | The helpers allocate nothing and return the upstream codes (-1 for NULL, 1 invalid, 0 valid), so every caller's error handling is unchanged; valgrind reports no leak. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | The helpers are pure functions of their arguments. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Same signatures as the upstream validators; XML_HIDDEN prototypes in both files. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Same single scan as upstream, with the table variant selected by a flag. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Invalid values fail with the existing XSD diagnostics (XSD-ERROR, PARSE-XML-EXCEPTION, XSD-SYNTAX-ERROR), checked in the qtest. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | design/xml-name-characters.md, the release note and EXECUTION.md describe the rule, the scope (XML names unchanged) and the decision. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No format strings from input; the probe uses bounded snprintf with length checks. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | qtest 5 cases/221 assertions pass and its strict cases fail on the previous build; the probe passes on the bundled library and fails on a library without the fix; 88 provider tests pass; the full suite passes (recorded in EXECUTION.md). |
