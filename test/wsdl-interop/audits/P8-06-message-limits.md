# P8-06 message and document limits audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to P8-06. Scope: SoapMessageLimits, SoapMessageLimitsScope, the decode budget and depth checks, array bounds, MIME part and WSDL/XSD loading limits in `qlib/WSDL.qm` with the wsdl_limits documentation and release notes; the limits options and setters of `qlib/SoapClient.qm`, `qlib/SoapClientIo/SoapClientIo.qc` (and `SoapClientIo.qm` notes) and `qlib/SoapHandler.qm`; and `test/soap-message-limits.qtest` with its interrupt helper `test/soap-message-limits-interrupt.qr`.
...
| 9 | Copyright 2026 on all new files | Pass | test/soap-message-limits.qtest, test/soap-message-limits-interrupt.qr and this record carry the 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | test/soap-message-limits.qtest uses %modern; the interrupt helper program uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/soap-message-limits.qtest is executable; the helper is parsed by the test, not run. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | test/soap-message-limits.qtest uses %prepend-module-path before its relative %requires; the helper uses a relative %requires resolved against its own path. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | Only QUnit, xml, HttpServer and in-repo modules. |
...
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | No new I/O; loading limits bound the existing imports. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Every limit approved by the user is implemented and enforced by default; the decompression bomb is a core root cause handed off with a documented gap, as approved; the particle-program performance defect is recorded for P9 rather than patched without safe invalidation. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Limits and budgets are thread-locals set by scope objects whose destructors restore them; the interruption test proves no state leaks after PROGRAM-INTERRUPTED in decoding, multipart parsing and loading; loading depth is restored with on_exit. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Per-message budgets are thread-local; configured limits are immutable hashes replaced atomically by setters. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Public hashdecl SoapMessageLimits; typed budget and scope classes; limit values are checked positive (SOAP-LIMIT-ERROR) and unknown keys are rejected by the hashdecl. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Budget checks are O(1) per value; depth checks are one iterative pass; padding and empty rows are built by doubling; the rank check makes the arrayType parse linear. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Every limit raises SOAP-DESERIALIZATION-ERROR (SOAP-MESSAGE-ERROR for MIME, WSDL-ERROR for loading) naming the limit; oversized digit strings are rejected before conversion. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | SoapMessageLimits, SoapMessageLimitsScope, WebService/WSOperation/SoapClient/SoapClientIo/SoapHandler getters and setters and the loading options are documented with @param/@return/@throw and examples; docs build with no warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | Hardening change; no credentials or format strings. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | soap-message-limits.qtest (13 cases) checks each limit at and beyond its value with exact counts, the measured attack inputs, every parsing path, configuration precedence including saved objects, and interruption; the full suite passes except the two core-blocked IEEE gates. |
