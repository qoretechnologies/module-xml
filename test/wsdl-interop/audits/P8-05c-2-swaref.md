# P8-05c-2 swaRef attachment references audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to P8-05c-2. Scope: the swaRef writer, resolver, scopes and XsdSwaRefHelper, the SwaPackageReferences refactoring, the value hooks in XsdDocumentValueHelper and XsdAttribute, message packaging and decoding in SoapBinding, XsdSimpleType::isAttachmentReference() and getLexicalDataProviderType() in `qlib/WSDL.qm` with the wsdl_swaref section and release note; `design/wsdl-multipart-input.md`; the new `test/soap-swaref.qtest` and `test/swaref.wsdl`; and the live CXF `echoDataRef` exchange in `test/wsdl-interop/swa-peer/` and `test_swa_parts.py`.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | The WSDL v0.5.8 release notes and the new wsdl_swaref section describe swaRef values. |
...
| 9 | Copyright 2026 on all new files | Pass | test/soap-swaref.qtest, test/swaref.wsdl and this record carry the 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | test/soap-swaref.qtest uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/soap-swaref.qtest is executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | test/soap-swaref.qtest uses %prepend-module-path before its relative %requires. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | Only QUnit, xml and HttpServer, which the modules themselves require. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | WS-I AP section 4.4 and R2928 are implemented for elements, attributes, simple content and typed parts in both directions, including Content-Location references. One limit is reported to the user for a decision instead of being worked around: retained XML (xml_values) consumers keep the URIs, and SoapHandler/SoapClient do not expose unbound parts to them, which needs a new API field. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Scopes restore their thread-locals in destructors; the writer is created per message, so the MTOM retry and failed serializations leave nothing behind; resolution errors raise SOAP-DESERIALIZATION-ERROR before any value is returned. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Per-message state lives in thread-locals set by scope objects; the per-type isAttachmentReference() cache is transient and idempotent, so concurrent first use computes the same value. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed hashdecl XopPartInfo parts, typed SwaPackageReferences members, typed provider union (binary|string). |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | The type check is cached per simple type; unused-part pruning is one substring search per created part; labels are built once per message. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Non-binary/string values raise SOAP-SERIALIZATION-ERROR; unknown cid:, fragment, unmatched, ambiguous and non-package references raise SOAP-DESERIALIZATION-ERROR citing R2928; malformed URIs fail anyURI validation. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | WSI_SWAREF_NS, isAttachmentReference() and getLexicalDataProviderType() are documented; wsdl_swaref user section, release note, design/wsdl-multipart-input.md and the swa-peer README updated; docs build with no warnings. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | soap-swaref.qtest (10 cases) covers the WS-I SendClaim shape in SOAP 1.1 and 1.2, media types and encodings, other senders' cid:/Content-Location forms, R2928 rejections, invalid values, non-reference lists/anyURI/XML attachments, MTOM together with swaRef, retained XML, provider types and a SoapHandler/SoapClient exchange; test_swa_parts.py exchanges CXF echoDataRef live in both directions. Testing found and fixed one defect in the change itself: list-of-swaRef provider types failed until list items and union members used the lexical provider type. |
