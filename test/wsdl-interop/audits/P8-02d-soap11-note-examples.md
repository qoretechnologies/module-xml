# P8-02d SOAP 1.1 Note examples audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to the uncommitted P8-02d changes on main `develop`
based on 91f1cc6. Scope: `qlib/WSDL.qm` (Namespaces::doType, encoding-namespace types, XsdArrayType generic arrays,
instance item types, rule 3(c) members, embedded ids, external references, XsdSimpleType xsi:type, member-type
retention, WSMessage::deserializeRpc), `test/soap11-note-examples.qtest`, `test/soap-encoded-references.qtest`,
`test/wsdl-interop/{encoded_corpus.py, test_axis_peer.py, normative/sources.json, encoded-corpus/*}`,
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | WSDL v0.5.8 release notes describe the encoding-namespace types, instance item types, generic arrays, element-named members, embedded ids, external references, member-type retention, the xsi:type correction and the part-type error. |
...
| 9 | Copyright 2026 on all new files | Pass | test/soap11-note-examples.qtest, encoded-corpus/soap11-note.wsdl and this record carry the 2026 copyright; soap11-note.json records the Note's own copyright line for its quoted examples. |
...
| 12 | `ns=Qore::XX` matches the QoreNamespace constructor path | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and test tooling. |
...
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | json is an external module and uses the %try-module pattern with an explicit MISSING-MODULE failure, as the other suites do; QUnit and xml are delivered with Qore and this repository. |
| 17 | No filesystem operations (fopen, open, creat, unlink, remove, rename, mkdir, rmdir, stat, chmod) without sandbox checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and test tooling. |
| 18 | No network operations (connect, bind, socket, getaddrinfo, gethostbyname) without sandbox checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and test tooling. |
| 19 | If filesystem/network ops exist, verify `QoreSandboxManagerHelper` usage | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and test tooling. |
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | No I/O added to WSDL.qm: references outside the message are rejected, never retrieved. The suite reads its corpus with ReadOnlyFile. The Note was downloaded once with TLS verification for extraction; the gate works offline. |
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and test tooling. |
| 22 | Uses `qore_check_cancel()` (NOT deprecated `qore_check_io_interrupt()`) | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and test tooling. |
| 23 | Check frequency: every 100 iterations for tight loops, every 10 for expensive iterations | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and test tooling. |
| 24 | No blocking operations without cancellation support | N/A | No C++ or QPP change; the changes are Qore module code in qlib/WSDL.qm, tests and test tooling. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Every gap the Note examples exposed is fixed at its root, including two pre-existing defects (the doType crash and the base-type xsi:type). The Note's errata are asserted as rejections, not corrected. The limits that remain are stated: SOAP-ENC:Struct is not provided (explicit WSDL-ERROR), and a generic array's struct members need preserve_types to keep their type on output. No TODOs or stubs. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | All new checks raise descriptive SOAP-DESERIALIZATION-ERROR, SOAP-SERIALIZATION-ERROR or WSDL-ERROR exceptions before producing values. Embedded ids are validated, duplicate-checked and cycle-checked before conversion. Slot allocation stays bounded by MaxSlots, and shape inference stops at empty dimensions, so it always terminates. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | No shared mutable state added: the generic array and encoding builtins are immutable after construction, and member type resolution creates independent builtin objects instead of populating shared schema caches. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed hashdecl shapes (XsdEncodedArrayShape), typed references and *XsdAbstractType results. The doType fix removes an assignment of NOTHING into a typed reference. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Member type resolution runs once per array level, not per member. Shape inference is linear in the value size. The full suite ran at its previous cost (392 parallel targets in 979 s). |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Each of these is rejected with a specific message: a missing generic arrayType, unknown or incompatible item types, mixed or inconsistent generic shapes, invalid, duplicate or cyclic embedded ids, external references, and unresolvable part types. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | design/soap-encoding.md (reference context, type names, arrays, type annotations, Note examples), the encoded-corpus README, the release notes, the documented constructor and helpers, and the PLAN.md and EXECUTION.md entries. Docs build with no warnings. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | No credentials or network access in decoding; constant format strings; the third-party document is not redistributed. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | All 27 instance examples are checked against the Note; each fix has negative cases. The full suite caught one over-reach (union xsi:type), which was fixed against XML Schema 1.0 section 3.14.6 and rechecked with every union suite and gate. Both Axis directions pass with the core fix applied to a scratch copy. |

Result: **18 Pass / 44 N/A / zero Fail**. The full suite found one regression before commit, the union `xsi:type`; it is fixed and rechecked.
