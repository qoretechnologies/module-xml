# P8-04 SOAP 1.2 RPC Representation audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to the uncommitted P8-04 changes on main `develop`
based on 675f7e0. Scope: `qlib/WSDL.qm` (WSOperation parameterOrder and getReturnPartName, SoapBinding
usesSoap12RpcRepresentation, rpc:result output and decoding, the one-child rule, rpc:BadArguments, nil and
absent encoded parameters, WSDLLib::toXmlName/fromXmlName, T59 subcode), `qlib/SoapHandler.qm`
(rpc:ProcedureNotPresent), `test/soap12-rpc.qtest`, `test/soap12-collection.qtest`, `design/soap-encoding.md`,
and PLAN.md and EXECUTION.md.
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | WSDL v0.5.8 and SoapHandler 0.3.4 release notes describe the RPC Representation, nil parameters, Appendix B and ProcedureNotPresent. |
...
| 9 | Copyright 2026 on all new files | Pass | test/soap12-rpc.qtest, test/soap12-collection.qtest and this record carry the 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | Both new suites use %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | Both new suites are executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | Both new suites use %prepend-module-path before relative %requires of WSDL.qm and SoapHandler.qm. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | soap12-collection.qtest uses the %try-module pattern for json; QUnit, xml and HttpServer are delivered with Qore. |
...
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | No I/O added to the modules; the suites use HttpServer and HTTPClient on 127.0.0.1 and read their corpus with ReadOnlyFile. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Complete for the RPC Representation's requirements. The scoping decisions are documented, not deferred: the representation applies with SOAP 1.2 Encoding only, and Appendix B is an API because WSDL names are already XML names. The collection gate records its 11 out-of-scope tests with their reasons, and its errata as expected rejections. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | rpc:result, one-child and argument errors raise SOAP-DESERIALIZATION-ERROR before callbacks. The BadArguments rethrow keeps the original description and is applied only to deserialization errors without more specific options; interruption errors pass through. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | servesSoap12Rpc() reads the method registry under the handler's read lock; everything else is per message. parameter_order is immutable after construction. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed *list<string> parameter_order, validated in deserializeMembers(), and a typed hash<SoapFaultOptions>; toXmlName() iterates list<char>. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | rpc:result handling is linear in the wrapper's children. servesSoap12Rpc() scans the registry only on the error path. The full suite ran at its previous cost (392 parallel targets in 955 s). |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Each rpc:result error names the edge or operation concerned, and the one-child error cites section 4.2.3. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | design/soap-encoding.md (RPC Representation, missing parameters, Appendix B names, W3C collection), Doxygen for getReturnPartName, usesSoap12RpcRepresentation, isNil, toXmlName and fromXmlName with examples, the release notes, and the PLAN.md and EXECUTION.md entries. Docs build with no warnings. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | The W3C collection's own requests and replies drive the gate, and Appendix B.2's examples drive the name mapping. Both suites fail against the P8-03b modules. The full suite on one runtime fails only on the two core-blocked IEEE gates. |
