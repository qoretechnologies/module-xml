# P5-20a native identity XPath audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied all 62 audit-changes checks to the changes based on `7a59f8f`.
The skill was reread before this commit; all five previously read guides have
unchanged verified hashes. Scope: native pattern grammar correction, behavioral
dependency selection, regression tests, documentation and acceptance evidence.
Main Qore is read-only; no installation or push. The unrelated
...
| 21 | All `for`/`while` loops that could iterate >100 times have `qore_check_cancel()` checks | N/A | No production loop added or traversal bound increased. Configure probe has 14 bounded schema cases; the direct C matrix has 88 fixed checks. The native C dependency has no Qore runtime dependency. |
...
| 24 | No blocking operations without cancellation support | Pass | No blocking production operation added; native pattern parsing remains local. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Directly fixes empty-input and terminal-union parser conditions. No source fixture rewriting or weakened grammar. WSDL identity compilation and instance tables are independently root-caused and assigned to the next P5 increment. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | New separator check precedes temporary allocation and uses existing cleanup of parser, prior pattern chain and temporary text. No ownership transfer. Direct ELF Valgrind has zero errors/lost bytes; direct C Valgrind frees all 486 allocations. Existing provider allocation regressions pass. |
...
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Native checks retain libxml2 pointer/status types; C-only pointer casts are appropriate. Constant snprintf format is bounds-checked. Qore fixture collections are typed. No public API or ABI change. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Two constant-time conditions; no extra traversal, recursion or copying in production. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | 54 schemas cover 22 valid and 32 invalid selector/field declarations. Three native APIs assert precise errors and unchanged DOM data. The safe C API checks null/empty input, invalid arms, four modes and dictionary/non-dictionary ownership. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Durable design/example, README, release notes, oracle evidence and inventory describe implemented behavior and remaining WSDL scope. No public method added. Native docs build cleanly. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | Constant bounded probe format and SHA-256-guarded source correction. Separator lookahead reads the next byte only after a non-NUL separator; initial empty check short-circuits null input. No credentials or new I/O. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | 54-schema native matrix and independent pinned Xerces checker pass with two explicitly classified normative whitespace differences root-caused in the oracle scanner. All 68 provider tests, 37 affected Qore suites and three Python supplements pass. Both corpus modes retain byte-identical reports. Both Valgrind runs are clean. |
