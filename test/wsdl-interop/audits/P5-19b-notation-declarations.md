# P5-19b WSDL notation declaration audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied the complete audit-changes checklist to changes based on `b8b006a`.
The skill was reread before commit. All five previously read design guides retain
their verified hashes. Scope: WSDL declaration storage, its public metadata API,
Qore/Python regressions, release notes, implemented design and acceptance records.
Main Qore is read-only; no push or install. Unrelated `test/cmake/__pycache__/`
is excluded. See [evidence](../notation-declarations-evidence.md) and
[inventory](../P5-19b-validation.json).
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | External WSDL v0.5.8 release notes describe retained notation identifiers and validation. |
...
| 9 | Copyright 2026 on all new files | Pass | All authored Qore, Python and Markdown files carry 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | New Qore suite and worker use %modern; standalone prototype is outside the committed scope. |
| 14 | Executable permission set (`chmod +x`) | Pass | New .qtest, .qr and Python suite are executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | Suite and worker prepend local qlib before relative WSDL requirements. Tests load local Debug XML. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | WSDL/XML are delivered by this project; QUnit is core. External json is guarded with an explicit missing-dependency error. |
...
| 20 | No `File::`, `Dir::`, `Socket::`, `HTTPClient::` usage without justification | Pass | Only test fixture file reads are added. Production identifiers are metadata and never fetched. |
...
| 37 | Fields with finite allowed values use `allowed_values` with `AllowedValueInfo` containing both `value` and `display_name` (Title Case, human-readable) — never bare values, never described only in text | N/A | No DataProvider action, request/response field, app, factory, JNI dependency or installation change. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Implements the independently testable declaration registry prerequisite without a workaround or fixture-specific production branch. NOTATION conversion and type-use checks remain explicitly assigned next within P5. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Maps are validated before publication; failed schema additions restore the prior shared notation/component/dependency state. Tests cover corrupted reconstruction, failed replacements, post-publication resolution errors, include retry and cancellation. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | Mutation is documented as construction-only. Returned hashes have value semantics; four event-synchronized independent reconstructed schemas retain their own maps. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Public XsdNotationInfo hashdecl and typed maps preserve field types. Untrusted reconstruction validates raw containers and fields before typed construction. No new untyped callbacks. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Declaration collection and publication are linear in declaration count, with hash lookup for duplicates and values. A 1024-declaration regression checks saved first/middle/last entries. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Positive/negative cases distinguish absent and empty identifiers, whitespace normalization, names, IDs, foreign/schema attributes, duplicate symbol names, malformed metadata and dependency rollback, with exact exception categories. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | Public methods have parameter/return/exception contracts as applicable. Durable design, realistic API example, release notes, README and evidence describe semantics and construction concurrency. Native/WSDL docs and WSDL metadata targets pass cleanly. |
...
| 61 | **Security**: No user-controlled format strings; no buffer overflows; bounds checking on array indices; no credentials in code | Pass | Constant format strings, validated XML characters and NCNames, typed metadata and bounded subprocess/event deadlines. No credentials or new production I/O. |
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | 167 suites and eight supplements pass. 391 focused assertions plus 54 independent schema cases verify metadata and pinned Xerces verdicts. Four corpus results are unchanged except the expected WSDL source hash. |
