# P8-05c-1 SOAP with Attachments references audit

Copyright (C) 2026 Qore Technologies, s.r.o.

Applied `/home/david/.claude/skills/audit-changes/SKILL.md` to P8-05c-1. Scope: `WSDLLib::substHref()` and its helpers and `WsdlMultipartHelper::read()` in `qlib/WSDL.qm` with the release note, `design/wsdl-multipart-input.md`, `design/soap-encoding.md`, the new `test/soap-swa-references.qtest`, and the pinned sources (`normative/sources.json`, `normative/wsiap10-requirements.json`, `wsi_requirements.py`, `test_attachment_sources.py`).
...
| 2 | Entry exists in `doxygen/lang/900_release_notes.dox.tmpl` (for modules in the Qore repo; external modules have release notes in their .qm) | Pass | The WSDL v0.5.8 release notes describe SwA reference resolution and the accepted Axis-style messages. |
...
| 9 | Copyright 2026 on all new files | Pass | test/soap-swa-references.qtest, normative/wsiap10-requirements.json and this record carry the 2026 copyright. |
...
| 13 | `%modern` directive present | Pass | test/soap-swa-references.qtest uses %modern. |
| 14 | Executable permission set (`chmod +x`) | Pass | test/soap-swa-references.qtest is executable. |
| 15 | Uses %prepend-module-path  before %requires for in-repo modules (Qore and Qore modules only; not Qorus) | Pass | test/soap-swa-references.qtest uses %prepend-module-path before ../qlib/WSDL.qm. |
| 16 | External module dependencies use `%try-module` — except modules delivered with the project itself (Qore ex: DataProvider, ConnectionProvider, QUnit, etc.) which use hard `%requires` | Pass | Only QUnit and xml, which the module itself requires. |
...
| 53 | **No workarounds**: No TODOs, FIXMEs, stubs, or partially-implemented features | Pass | Implements SwA Note section 3 completely: base URI order, cid: and Content-Location labels, same-document references and unmatched references. swaRef (WS-I AP R2928) is the separate plan item P8-05c-2. |
| 54 | **Exception safety**: C++ uses `ReferenceHolder` for Qore allocations, `std::unique_ptr` for C++ allocations, `*xsink` checked after every fallible operation | Pass | Resolution works on the caller's parsed copy; every error raises SOAP-MESSAGE-ERROR before the message is used. |
| 55 | **Thread safety**: All mutable shared state protected by `std::lock_guard<std::mutex>` or documented as immutable-after-construction | Pass | No shared state; labels are built per message. |
| 56 | **Type safety**: Strongly-typed `code<return(args)>` instead of untyped `code`; `static_cast` instead of C casts; typed hashdecls for results; enums where appropriate | Pass | Typed label map hash<string, list<hash<auto>>>; typed *data results. |
| 57 | **Performance**: No O(n²) where O(n) is possible; no unnecessary copies; coordinate descent uses incremental residuals not full matrix multiply | Pass | Labels are built once per message, and each element is visited once; replaced content is not searched again. |
| 58 | **Error handling**: All inputs validated (dimensions, empty data, unfitted models); C++ I/O handles EAGAIN/EINTR if applicable | Pass | Unknown cid: parts, malformed percent-encoding, ambiguous Content-Location labels and repeated Content-Location headers (part or package) raise SOAP-MESSAGE-ERROR. |
| 59 | **Documentation**: Doxygen `@param`, `@return`, `@throw` on all public methods; `@par Example` with realistic business scenarios; `@note` for important caveats | Pass | parseMultiPartSOAPMessage() documents the new package_location key; design/wsdl-multipart-input.md and soap-encoding.md describe the rules; release note added. |
...
| 62 | **Correctness**: Algorithms verified against reference implementations; edge cases tested (empty data, single sample, all-zero features) | Pass | soap-swa-references.qtest covers the Note's section 3 examples, RFC 2392 percent-encoding and scheme case, headers, nesting, repeated references, xml:base and the encoded Axis case, which the previous module rejected as a "non-local part". The Note is pinned by digest with its quotes checked against a retrieved copy; WS-I AP 1.0 is pinned as a reproducible requirement extract. |
