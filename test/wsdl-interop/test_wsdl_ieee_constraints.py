...
"""Exact IEEE values through saved services and both real SOAP binding models."""
import json
import math
import os
...
from test_ieee_constraints import fixtures, FIXTURE
from survey import SOAP_NAMESPACES


def meaning(node, model):
    name = model['name']
    wide = name.startswith('double') or name.startswith('control/singleton-list') or name.startswith('control/INF')
    ...
    def scalar(text):
        ...
        if text == 'NaN':
            return 'IEEE', 'NaN'
        value = {'INF': math.inf, '-INF': -math.inf}.get(text)
        if value is None:
            value = rounded_value(text, wide)
        # XSD 1.0 has a single zero value; every nonzero IEEE bit is significant.
        return 'IEEE', struct.pack('!d', 0.0 if value == 0 else value)
...
class WsdlIeeeConstraintsTest(unittest.TestCase):
    def test_source_saved_providers_and_both_bindings(self):
        ...
        worker = Path(os.environ.get('QORE_IEEE_CONSTRAINT_WORKER',
        ...
        self.assertEqual(2148, len(expected))
        ...
        jobs = {m['name']: SchemaJob(m['name'], f'http://example.invalid/wsdl-ieee/{i}.xsd',
        ...
        for row in rows:
            ...
            with self.subTest(name=row['name'], stage=row['stage']):
                ...
                payloads += 1
        self.assertEqual(1248, payloads)
        oracle = run(list(jobs.values()))
        self.assertEqual(set(jobs), set(oracle['schemas']))
        self.assertEqual(1548, len(oracle['documents']))
        differences = []
        for model in models:
            ...
            for key in jobs[model['name']].documents:
                self.assertEqual(True if model['oracle_valid'] else None, oracle['documents'][key]['ok'], key)
        ...
        self.assertEqual(48, len(independent_instances['schemas']))
        self.assertEqual(432, len(independent_instances['documents']))
