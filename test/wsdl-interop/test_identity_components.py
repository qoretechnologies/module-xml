#!/usr/bin/env python3
"""Independent XSD 1.0 identity component relationships.
...
class IdentityComponentsTest(unittest.TestCase):
    def test_pinned_xerces(self):
        models = json.loads((Path(__file__).parent / 'fixtures/identity-components.json').read_text())
        self.assertEqual(30, len(models))
        self.assertEqual(14, sum(m['schema_valid'] for m in models))
        self.assertEqual(len(models), len({m['name'] for m in models}))
        report = run([SchemaJob(m['name'], f'http://example.invalid/identity-components/{i}.xsd',
